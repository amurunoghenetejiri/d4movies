...
import { useAllMovies, useGenres } from "@/lib/movies";
  ...
  return (
    <AppShell>
      ...
      <div className="mx-auto max-w-7xl px-4 md:px-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
        {genres.map((g, i) => {
          const cover = all.find((m) => m.genres.includes(g))?.backdrop;
          return (
            <Link key={g} to="/search" search={{ q: g }} className="group relative aspect-[4/3] overflow-hidden rounded-2xl border border-border hover-lift">
              {cover && <img src={cover} alt={g} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" />}
